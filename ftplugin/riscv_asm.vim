...
if !exists("b:riscv_asm_all_enable")
    ...
    if s:riscv_asm_isa =~ '\c^-\=zacas\(\d\+\(\.\d\+\)\=\)\=\(-\|$\)'
        let s:extract_version = substitute(s:riscv_asm_isa, '\c^-\=zacas\(\d\+\(\.\d\+\)\=\)\=.*', '\1', "")
        if s:extract_version !~ '\d\+\.\d\+'
            let b:riscv_asm_zacas = b:riscv_asm_zacas_max
        else
            let b:riscv_asm_zacas = str2float(s:extract_version)
            if b:riscv_asm_zacas > b:riscv_asm_zacas_max
                let b:riscv_asm_zacas = b:riscv_asm_zacas_max
            endif
        endif
        let s:riscv_asm_isa = substitute(s:riscv_asm_isa, '\c^-\=zacas\(\d\+\(\.\d\+\)\=\)\=', "", "")
    else
        if exists("b:riscv_asm_zacas")
            unlet b:riscv_asm_zacas
        endif
    endif
    " Zawrs extension
